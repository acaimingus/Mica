 ...
 *
 * Description: Implementation of terminal launcher helper to ensure MicaPairingService runs inside a visible terminal
 * emulator.
 */
...
void TerminalLauncher::EnsureTerminalWindow(const int argc, char *argv[])
{
    ...
    const std::vector<TermCandidate> candidates = {{"/usr/bin/gnome-terminal", {"--wait", "--"}},
                                                   {"/usr/bin/ptyxis", {"--wait", "--"}},
