...
void NotificationManager::Initialize()
{
    ...
    std::thread glibThread([loop]() { g_main_loop_run(loop); });
    glibThread.detach();
}
...
bool NotificationManager::RequestDesktopPairingNotification(const Network::NetworkConfig &config)
{
    ...
    NotifyNotification *n = notify_notification_new("Mica: New devices found",
    ...
    auto actionFree = [](const gpointer user_data) { delete static_cast<std::shared_ptr<SyncData> *>(user_data); };
    ...
    g_signal_connect_data(
        n, "closed", G_CALLBACK(+[](NotifyNotification *, gpointer user_data) {
        ...
        userDataClosed,
        [](gpointer user_data, GClosure *) { delete static_cast<std::shared_ptr<SyncData> *>(user_data); },
