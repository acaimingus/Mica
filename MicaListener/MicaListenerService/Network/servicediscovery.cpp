...
void ServiceDiscovery::CreateServiceBrowser(AvahiClient *_client)
{
    ...
    browser.reset(avahi_service_browser_new(_client, AVAHI_IF_UNSPEC, AVAHI_PROTO_INET, serviceName.c_str(), nullptr,
...
void ServiceDiscovery::CreateServiceResolver(AvahiClient *_client, const AvahiIfIndex _interface,
                                             const AvahiProtocol _protocol, const char *_name, const char *_type,
                                             const char *_domain)
{
    ...
    std::unique_ptr<AvahiServiceResolver, AvahiServiceResolverDeleter> newResolver(
        avahi_service_resolver_new(_client, _interface, _protocol, _name, _type, _domain, AVAHI_PROTO_INET,
                                   static_cast<AvahiLookupFlags>(0), ResolveCallback, this));
    if (!newResolver)
    {
        std::cerr << logName << "Failed to create the Avahi Service Resolver for " << (_name ? _name : "(null)")
                  << std::endl;
    }
...
void ServiceDiscovery::BrowseCallback([[maybe_unused]] AvahiServiceBrowser *_browser,
                                      [[maybe_unused]] AvahiIfIndex _interface,
                                      [[maybe_unused]] AvahiProtocol _protocol, const AvahiBrowserEvent _event,
                                          ...
                                          const char *_domain)
{
    ...
    case AVAHI_BROWSER_NEW:
        ...
        std::clog << "Found service: " << (_name ? _name : "(null)") << " type: " << (_type ? _type : "(null)")
    ...
    case AVAHI_BROWSER_REMOVE:
        std::clog << "Removed service: " << (_name ? _name : "(null)") << " type: " << (_type ? _type : "(null)")
                  << " domain: " << (_domain ? _domain : "(null)") << std::endl;
        if (_name != nullptr)
        {
...
void ServiceDiscovery::ResolveCallback([[maybe_unused]] AvahiServiceResolver *_resolver,
                                       [[maybe_unused]] AvahiIfIndex _interface,
                                       [[maybe_unused]] AvahiProtocol _protocol, AvahiResolverEvent _event,
                                       const char *_name, [[maybe_unused]] const char *_type,
                                       [[maybe_unused]] const char *_domain, [[maybe_unused]] const char *_hostName,
                                       const AvahiAddress *_address, uint16_t _port,
                                       [[maybe_unused]] AvahiStringList *_text,
                                       [[maybe_unused]] AvahiLookupResultFlags _flags, void *_userData)
{
                                           ...
                                           uint16_t _port) const
{
    ...
    case AVAHI_RESOLVER_FOUND:
        ...
        std::clog << "Avahi Service Resolver found the Service: " << _name << " / " << address << " / " << _port
                  << std::endl;
        if (_name != nullptr)
        {
