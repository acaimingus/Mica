...
std::optional<Network::NetworkConfig> PairingManager::HandleDevicePairing(const Network::NetworkConfig &newDeviceConfig,
                                                                          const Network::DeviceRegistry &deviceRegistry)
{
    ...
    pairingSocketServer.Start(
        ...
        [currentSecret, currentSock](const std::string & /*name*/, const std::string &ip,
                                     const uint16_t port) -> std::string {
    ...
    if (!g_spawn_async(nullptr, cArgs.data(), nullptr, G_SPAWN_DO_NOT_REAP_CHILD, nullptr, nullptr, &childPid,
                       &spawnError))
    {
        std::cerr << logName << "Failed to spawn mica-pairing: " << (spawnError ? spawnError->message : "unknown")
                  << std::endl;
        if (spawnError)
